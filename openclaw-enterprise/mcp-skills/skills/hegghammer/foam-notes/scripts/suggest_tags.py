...
def extract_existing_tags(foam_root: Path) -> dict:
    ...
    return {
        'counts': dict(tags_counter),
        'locations': tag_locations
    }


def extract_content_tags(content: str) -> set:
    ...
    # Filter for potential tags (nouns, key concepts)
    ...
    stop_words = {
        ...
        'under', 'again', 'further', 'once', 'me', 'him', 'them', 'us',
        'can', 'may', 'might', 'must', 'shall', 'should', 'from', 'as',
        'about', 'into', 'through', 'during', 'before', 'after', 'above',
        'below', 'between', 'among', 'within', 'without', 'against',
        'notes', 'note', 'foam', 'markdown', 'file', 'page', 'section'
    }
...
def suggest_tags(note_content: str, existing_tags: dict, min_count: int = 1) -> dict:
    """Suggest tags for a note."""
    content_tags = extract_content_tags(note_content)
...
def format_suggestions(suggestions: dict, existing_tags: dict) -> str:
    ...
    lines.append("")
    lines.append("Respond with numbers to add (e.g., '1 3 5'), 'all', 'existing', 'new', or 'none'")
...
def main():
    ...
    parser.add_argument('note', help='Path to note file (relative to foam_root or absolute)')
    parser.add_argument('--foam-root', help='Foam workspace root directory (overrides config)')
    parser.add_argument('--min-count', '-m', type=int, default=1,
                        help='Minimum usage count for existing tags (default: 1)')
    parser.add_argument('--apply', '-a', help='Comma-separated numbers to apply (e.g., "1,3,5")')
    ...
    suggestions = suggest_tags(content, existing_tags, args.min_count)
    ...
    if args.apply:
        try:
            ...
            tags_to_add = [all_tags[i-1] for i in selections if 1 <= i <= len(all_tags)]
