...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate videos via AIMLAPI video endpoints")
    parser.add_argument("--prompt", required=True, help="Text prompt for the video")
    parser.add_argument("--model", required=True, help="Video-capable model reference")
    parser.add_argument("--endpoint", default="videos/generations", help="Endpoint path (relative)")
    ...
    parser.add_argument("--output-extension", default="mp4", help="File extension for b64 outputs")
...
def main() -> None:
    ...
    if not data:
        raise SystemExit("No videos returned. Check model access and payload.")

    for index, item in enumerate(data, start=1):
        if "b64_json" in item:
            extension = args.output_extension.lstrip(".")
            file_path = out_dir / f"video-{index}.{extension}"
            write_from_b64(item["b64_json"], file_path)
        elif "url" in item:
            url_value = item["url"]
            extension = pathlib.Path(url_value).suffix or f".{args.output_extension}"
            file_path = out_dir / f"video-{index}{extension}"
            download_url(url_value, file_path)
        else:
            raise SystemExit(f"Unsupported response item: {item}")

    print(f"Saved {len(data)} video(s) to {out_dir}")
