...
const DEFAULT_WS_RPC = 'wss://rpc.starknet.lava.build/ws';
const DEFAULT_HTTP_RPC = 'https://rpc.starknet.lava.build';
...
function createCronJob(config) {
  ...
  const jobName = config.schedule?.name || `watch-smart-${Date.now()}`;
...
class SmartEventWatcher {
  constructor(config) {
    this.config = config;
    this.wsUrl = config.wsRpcUrl || DEFAULT_WS_RPC;
    this.httpUrl = config.httpRpcUrl || DEFAULT_HTTP_RPC;
    ...
    // Polling state
    this.provider = new Provider({ nodeUrl: this.httpUrl });
  ...
  async poll() {
    ...
    try {
      ...
      if (latestNumber > this.currentBlock) {
        for (let blockNum = this.currentBlock + 1; blockNum <= latestNumber; blockNum++) {
          await this.checkBlock(blockNum);
        }
        this.currentBlock = latestNumber;
      }
    } catch (err) {
  ...
  handleEvent(event, source) {
    ...
    if (this.webhookUrl) {
      sendWebhook(this.webhookUrl, eventData);
    }
  }
  ...
  startHealthCheck() {
    this.healthCheckTimer = setInterval(() => {
      ...
      if (currentMode === 'websocket') {
        ...
        if (this.forcedMode === 'auto' && this.wsReconnectAttempts < 3) {
