...
- Require Shipping: ${p.require_shipping}`);
        ...
        if (!namedArgs.name || !namedArgs.price) {
          console.error("Usage: products create --name \"Name\" --price 1000 [--description \"...\"]");
          return;
        }
        ...
        if (data.success) console.log(`✅ Created: ${data.product.name} (ID: ${data.product.id})`);
        ...
        if (data.success) console.log(`✅ Updated: ${data.product.name}`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'delete') {
        if (!namedArgs.id) { console.error("Error: --id required"); return; }
        const data = await apiRequest('DELETE', `/products/${namedArgs.id}`);
        if (data.success) console.log(`✅ Deleted Product (ID: ${namedArgs.id})`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'enable') {
        if (!namedArgs.id) { console.error("Error: --id required"); return; }
        const data = await apiRequest('PUT', `/products/${namedArgs.id}/enable`);
        if (data.success) console.log(`✅ Published: ${data.product.name}`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'disable') {
        if (!namedArgs.id) { console.error("Error: --id required"); return; }
        const data = await apiRequest('PUT', `/products/${namedArgs.id}/disable`);
        if (data.success) console.log(`✅ Unpublished: ${data.product.name}`);
        else console.error("Failed:", data);
      }
    }
    ...
    // --- SALES ---
      ...
      if (subCommand === 'list' || !subCommand) {
        ...
        if (namedArgs.product_id) queryParams.push(`product_id=${namedArgs.product_id}`);
        ...
        if (data.success) console.log(`✅ Refund processed for Sale ID: ${namedArgs.id}`);
        else console.error("Failed:", data);
      }
        ...
        if (data.success) console.log(`✅ Marked as Shipped: ${data.sale.product_name} (Tracking: ${data.sale.tracking_url || 'None'})`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'resend-receipt') {
        if (!namedArgs.id) { console.error("Error: --id required"); return; }
        const data = await apiRequest('POST', `/sales/${namedArgs.id}/resend_receipt`);
        if (data.success) console.log(`✅ Receipt Resent for Sale ID: ${namedArgs.id}`);
        else console.error("Failed:", data);
      }
    }
    ...
    // --- LICENSES ---
       ...
       if (!namedArgs.product || !namedArgs.key) {
           console.error("Error: --product <id> and --key <license_key> required");
           return;
       }
...
Subscription: ${p.subscription_cancelled_at ? 'Cancelled' : (p.subscription_failed_at ? 'Failed' : 'Active')}`);
               ...
               console.error("Failed to verify license:", data);
           }
       }
       else if (subCommand === 'enable') {
           ...
           if (data.success) console.log(`✅ License Enabled: ${namedArgs.key}`);
           else console.error("Failed:", data);
       }
           ...
           if (data.success) console.log(`📉 Usage Decremented. New Count: ${data.uses}`);
           else console.error("Failed:", data);
       }
           ...
           if (data.success) console.log(`🔄 Key Rotated.\nNew Key: ${data.purchase.license_key}`);
           else console.error("Failed:", data);
       }
    }
...
- Universal: ${o.universal || 'false'}`);
        ...
        if (data.success) console.log(`✅ Created: ${data.offer_code.name} (ID: ${data.offer_code.id})`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'update') {
        if (!namedArgs.id) { console.error("Error: --id <discount_id> required"); return; }
        ...
        if (data.success) console.log(`✅ Updated: ${data.offer_code.name}`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'delete') {
        if (!namedArgs.id) { console.error("Error: --id <discount_id> required"); return; }
        const data = await apiRequest('DELETE', `/products/${namedArgs.product}/offer_codes/${namedArgs.id}`);
        if (data.success) console.log(`✅ Deleted Discount (ID: ${namedArgs.id})`);
        else console.error("Failed:", data);
      }
    }
...
- Processor: ${p.payment_processor}`);
    ...
    // --- VARIANT CATEGORIES ---
      ...
      if (subCommand === 'list') {
        ...
        if (data.success) console.log(`✅ Created Category: ${data.variant_category.title} (ID: ${data.variant_category.id})`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'update') {
        ...
        if (data.success) console.log(`✅ Updated Category: ${data.variant_category.title}`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'delete') {
        ...
        if (data.success) console.log(`✅ Deleted Category (ID: ${namedArgs.id})`);
        else console.error("Failed:", data);
      }
    }
    ...
    // --- VARIANTS ---
      ...
      if (subCommand === 'list') {
        ...
        if (data.success) console.log(`✅ Created Variant: ${data.variant.name} (ID: ${data.variant.id})`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'update') {
        if (!namedArgs.id) { console.error("Error: --id <id> required"); return; }
        ...
        if (data.success) console.log(`✅ Updated Variant: ${data.variant.name}`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'delete') {
        if (!namedArgs.id) { console.error("Error: --id <id> required"); return; }
        ...
        if (data.success) console.log(`✅ Deleted Variant (ID: ${namedArgs.id})`);
        else console.error("Failed:", data);
      }
    }
    ...
    // --- CUSTOM FIELDS ---
      ...
      if (subCommand === 'list') {
        ...
        if (data.success) console.log(`✅ Created Field: ${data.custom_field.name}`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'update') {
        if (!namedArgs.name) { console.error("Error: --name <name> required"); return; }
        ...
        if (data.success) console.log(`✅ Updated Field: ${data.custom_field.name}`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'delete') {
        if (!namedArgs.name) { console.error("Error: --name <name> required"); return; }
        ...
        if (data.success) console.log(`✅ Deleted Field: ${namedArgs.name}`);
        else console.error("Failed:", data);
      }
    }
      ...
      if (subCommand === 'list' || !subCommand) {
        ...
        if (data.success) console.log(`✅ Subscribed [${data.resource_subscription.resource_name}]: ${data.resource_subscription.post_url}`);
        else console.error("Failed:", data);
      }
      else if (subCommand === 'delete') {
        ...
        if (data.success) console.log(`✅ Deleted Subscription (ID: ${namedArgs.id})`);
        else console.error("Failed:", data);
      }
    }
