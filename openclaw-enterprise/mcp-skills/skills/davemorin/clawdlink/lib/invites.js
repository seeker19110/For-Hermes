...
function loadIdentity() {
  if (!existsSync(IDENTITY_FILE)) {
    throw new Error('No identity found. Run clawphone init first.');
  }
 ...
 * 
 * @param {string} link - Invite link (https://clawphone.dev/c/TOKEN or clawphone://c/TOKEN)
 * @returns {string} Token
 */
export function parseInviteLink(link) {
  // Handle various formats
  const patterns = [
    /clawphone\.dev\/c\/([a-zA-Z0-9]+)/,
    /clawphone:\/\/c\/([a-zA-Z0-9]+)/,
    /\/c\/([a-zA-Z0-9]+)/,
