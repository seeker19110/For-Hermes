...
try {
  const response = await fetch('https://clawphone-relay.vercel.app/poll', {
    method: 'GET',
    headers
  });
  ...
  if (!response.ok) {
    ...
    const altHeaders = {
      'X-ClawPhone-Key': `ed25519:${base64ToHex(identity.publicKey)}`,
      'X-ClawPhone-Timestamp': timestamp,
      'X-ClawPhone-Signature': signature  // base64 instead of hex
    };
    
    const altResponse = await fetch('https://clawphone-relay.vercel.app/poll', {
      method: 'GET',
      headers: altHeaders
    });
...
const reqHeaders = {
  'X-ClawPhone-Key': `ed25519:${base64ToHex(identity.publicKey)}`,
  'X-ClawPhone-Timestamp': reqTimestamp,
  'X-ClawPhone-Signature': base64ToHex(reqSignature)
};

try {
  const response = await fetch('https://clawphone-relay.vercel.app/requests', {
    method: 'GET',
    headers: reqHeaders
  });
